...
2. **Set two env vars** (Environment Variables tab):
   - `WC_API_KEY` — your Wispers Connect API key.
   - `SHARES` — one or more shares, `;`-separated, each `name | display | <container>:<port>`.
     `<container>` is the app's container name (Coolify names it `<service>-<uuid>`).
...
- **Adding an app:** append `; name | display | container:port` to `SHARES`, tick the
